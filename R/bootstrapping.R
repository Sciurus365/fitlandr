...
summarize_mean_potential_hessian <- function(object,
  ...
  per_point <- dplyr::bind_rows(lapply(seq_len(object$n_boot), function(i) {
    ...
    df_run$cluster <- assign_run_to_reference(df_run[, c("x", "y"), drop = FALSE], ref_xy)
...
#' @param min_barrier Minimum barrier height fraction used by `find_loc_min()` to classify minor minima. Default 0.05.
#' @param clustering_method Clustering backend for pooled minima. One of
#'   `"hdbscan"` (default), `"hungarian"`, `"gmm_bic"`, `"mean_potential_hessian"`.
...
#' of the 2×2 covariance matrix per cluster. For `"mean_potential_hessian"`, minima are first
...
summary.bootstrap_2d_ld <- function(object,
                                    exclude_minor = TRUE,
                                    min_barrier = 0.05,
                                    clustering_method = c("hdbscan", "hungarian", "gmm_bic", "mean_potential_hessian"),
                                    minPts = 5,
                                    level = 0.95,
                                    one_per_run = TRUE,
                                    ...) {
  ...
  clustering_method <- rlang::arg_match0(clustering_method, c("hdbscan", "hungarian", "gmm_bic", "mean_potential_hessian"))
...
cluster_bootstrap_minima <- function(boot_min_df, object, exclude_minor, min_barrier, clustering_method, minPts) {
