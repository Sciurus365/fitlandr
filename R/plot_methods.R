...
#'
#' Plot a 2D vector field estimated by [fit_vf_2d()]. Powered by [ggplot2::ggplot()].
#'
#' @param x A `vectorfield` object estimated by [fit_vf_2d()].
...
#'
#' Plot a 2D landscape estimated by [fit_vfld_2d()]. Powered by [ggplot2::ggplot()].
#'
#' @param x A `2d_vf_landscape` object created by [fit_vfld_2d()].
#' @param show_vectorfield Should the vector field also be plotted? If so, the landscape will be plotted below the vector field. `FALSE` by default.
#' @param show_nongrad_vectorfield Should the non-gradient part of the vector field be plotted? If so, it will be plotted at the top layer. `FALSE` by default.
#' @param nongrad_vectorfield_enlarge A number. How many times should the non-gradient vectors be enlarged on the plot?
#' @param nongrad_vectorfield_options A list passing other customized parameters to [ggplot2::geom_segment()] to control the vectors representing the non-gradient part of the vector field. Sky blue by default.
#' @param type One of "landscape" or "error"
#' @param ... Other parameters passed to [plot.vectorfield()].
#' @inheritParams plot.vectorfield
#'
#' @export
plot.2d_vf_landscape <- function(x, type = c("landscape", "error"),
																 show_vectorfield = FALSE,
																 show_nongrad_vectorfield = FALSE,
																 nongrad_vectorfield_enlarge = 1,
																 nongrad_vectorfield_options = list(color = "skyblue"),
  			...
  			c(
  				list(
  					...
  					mapping = ggplot2::aes(
  						x = x,
  						y = y,
  						xend = x + vx * nongrad_vectorfield_enlarge,
  						yend = y + vy * nongrad_vectorfield_enlarge
  					),
  					arrow = arrow
  				),
  				nongrad_vectorfield_options
  			)
  		)
  }
