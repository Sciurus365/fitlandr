...
struct Node {
	int index;
	double dist;
	// We want a min-heap, so we flip the comparison
...
List dijkstra(NumericMatrix d, IntegerVector s, IntegerVector e) {
	...
	// 0-based indexing for start and end
	int start_r = s[0] - 1;
	int start_c = s[1] - 1;
	int end_r = e[0] - 1;
	int end_c = e[1] - 1;
	...
	// Pre-calculate neighbor offsets (8-connectivity)
	...
	while (!pq.empty()) {
		...
		if (d_u > dists[u]) continue;
		if (u == end_idx) break;

		int r = u % nr;
		int c = u / nr;

		// Calculate gradient magnitude for current node (v)
		// using the logic from your original get_gradient_magnitude
		double dx, dy;
		if (r == 0) dx = d(r + 1, c) - d(r, c);
		else if (r == nr - 1) dx = d(r, c) - d(r - 1, c);
		else dx = (d(r + 1, c) - d(r - 1, c)) / 2.0;

		if (c == 0) dy = d(r, c + 1) - d(r, c);
		else if (c == nc - 1) dy = d(r, c) - d(r, c - 1);
		else dy = (d(r, c + 1) - d(r, c - 1)) / 2.0;
	...
	List path;
	int curr = end_idx;
	if (parent[curr] == -1 && curr != start_idx) return path; // No path found
