...
test_that("compare_minima_depths validates selected minima", {
  object <- structure(
    list(
      params = list(level = 0.95),
      per_minimum = data.frame(minimum = 1L, mean_U = 0),
      bootstrap_minima = data.frame(boot_index = 1L, minimum = 1L, U = 0)
    ),
    class = "summary_bootstrap_2d_ld"
  )

  expect_error(compare_minima_depths(object, c(1, 2)), "Missing minimum")
